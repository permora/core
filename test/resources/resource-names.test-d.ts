...
import type { PermissionBuilder } from '../../src/permissions';
...
const resourcesFromEnum = defineResources({
  [ResourceNameEnum.Project]: defineResource<Project>({
    actions: ['read', 'update', 'delete'],
  }),
  [ResourceNameEnum.Invoice]: defineResource<Invoice>({
    actions: ['read', 'approve'],
  }),
});
...
describe('strongly typed resource names', () => {
  ...
  it('accepts ResourceNames constants in session.can', async () => {
    ...
    const permissions = permissionBuilder(resourcesFromConstNames, {
      viewer: { project: ['read'] },
    });
